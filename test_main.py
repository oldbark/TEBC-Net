...
def compute_reach_dic(matched_mid):
    ...
    with open(os.path.join(args.output, 'transE_train.txt'), 'r',encoding='utf-8') as f:
        for line in f:
...
for fname in ["train.txt", "valid.txt"]:
    with open(os.path.join(args.output, fname), 'r',encoding='utf-8') as f:
        for line in f:
            items = line.strip().split("\t")
...
for line in open(os.path.join(args.output, 'test.txt'), 'r',encoding='utf-8'):
    items = line.strip().split("\t")
...
for line in open(os.path.join(args.output, 'cleanedFB.txt'), 'r',encoding='utf-8'):
    items = line.strip().split("\t")

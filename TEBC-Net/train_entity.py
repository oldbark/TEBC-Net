...
from Transformer_train import TransformerEncoder
from embedding import EmbedVector
from  lstm_transformer import lstm_TransformerEncoder
from  lstm_selfattention_mean import  lstm_selfattention_mean
from evaluation import get_names_for_entities
from sklearn.metrics.pairwise import euclidean_distances
from  embeddingplus import  embeddingplus
from  bigru_encoder import  bigru_encoder
from bigru_multihead import bigru_multihead
from bi_bigru_ffn import  bi_bigru_ffn
from cnn import  cnn
...
parser.add_argument('--rnn_fc_dropout', type=float, default=0.1)
...
for line in open(os.path.join(args.output, 'entity2id.txt'), 'r',encoding='utf-8'):
...
while True:
    ...
    for batch_idx, batch in enumerate(train_iter):
        ...
        model.train()
        optimizer.zero_grad()
        loss = criterion(model(batch), entities_emb[batch.mid.data, :])#.mid.data, :
        loss.backward()
        # clip the gradient
        ...
        if iterations % args.dev_every == 0:
            ...
            if curr_accu > best_model:
                ...
                torch.save(model, os.path.join(args.output, args.best_prefix + '_best_model_translstm_entity.pt'))
            else:

...
from qktest import qktest
from  cnn import  cnn
from bilstm import  bilstm
from singlecnn import singlecnn
from head2Tfbilstm import head2Tfbilstm

from TenMonthtest1 import Tentest1
from TenMonthtest2 import Tentest2

parser = ArgumentParser(description="Training predicate vector learning")
# parser.add_argument('--qa_mode', type=str, required=True, help='options are GRU, LSTM,CNN')
...
for line in open(os.path.join(args.output, 'relation2id.txt'), 'r',encoding='utf-8'):
...
while True:
    ...
    for batch_idx, batch in enumerate(train_iter):
        ...
        transvector = predicates_emb[batch.mid, :]



        ques_vector = model(batch)

        loss = criterion(ques_vector,transvector)
        loss.backward()
        # clip the gradient
        ...
        if iterations % args.dev_every == 0:
            ...
            if curr_accu > best_accu:  # total_loss < best_model
                ...
                torch.save(model, os.path.join(args.output, args.best_prefix + '_best_model_cnntest.pt'))
            else:

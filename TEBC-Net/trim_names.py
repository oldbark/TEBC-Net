...
def get_fb_mids_set(cleanfile, fbsubset):
    ...
    mids = []
    for i, line in enumerate(open(fbsubset, "r",encoding='utf-8')):
        if i % 1000000 == 0:
            print("line: {}".format(i))
...
def findsetgrams(dataset):
    ...
    for fname in ["annotated_fb_data_valid", "annotated_fb_data_test"]:
        for i, line in enumerate(open(os.path.join(dataset, fname + ".txt"), 'r',encoding='utf-8')):
            items = line.strip().split("\t")
            if len(items) != 4:
                print("ERROR: line - {}".format(line))
                break
...
def get_all_entity_mids(fbpath, entiset):
    ...
    #mids_dic = {}
    relat = []
    trainfile = open(os.path.join(args.output, 'transE_train.txt'), 'w',encoding='utf-8')
    validfile = open(os.path.join(args.output, 'transE_valid.txt'), 'w',encoding='utf-8')
    testfile = open(os.path.join(args.output, 'transE_test.txt'), 'w',encoding='utf-8')
    with open(fbpath, 'r',encoding='utf-8') as f:
    ...
    validfile.close()
    testfile.close()
    with open(os.path.join(args.output, 'entity2id.txt'), 'w',encoding='utf-8') as outfile:
    ...
    outfile.close()
    with open(os.path.join(args.output, 'relation2id.txt'), 'w',encoding='utf-8') as outfile:
...
if __name__ == '__main__':
    ...
    with open(os.path.join(args.dataset, "annotated_fb_data_train.txt"), 'r',encoding='utf-8') as f:
        for i, line in enumerate(f):
            items = line.strip().split("\t")
            if len(items) != 4:
                print("ERROR: line - {}".format(line))
    ...
    outfile = open(os.path.join(args.output, 'names.trimmed.txt'), 'w',encoding='utf-8')  # output file path for trimmed names file

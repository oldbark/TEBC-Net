...
from  trans_entity_detection import  trans_entity_detection
from  translstm_entitydetection import translstm_entitydetection
...
parser.add_argument('--output_channel', type=int, default=600)
...
outfile = open(os.path.join(args.output, 'dete_train.txt'), 'w',encoding='utf-8')
...
model = translstm_entitydetection(config)
    ...
    for batch_idx, batch in enumerate(train_iter):
        ...
        if iterations % args.dev_every == 0:
            ...
            if R > best_dev_R:
                best_dev_R = R
                iters_not_improved = 0
                snapshot_path = os.path.join(args.output, args.dete_prefix + '_best_model_translstm.pt')
